...
main(int argc, char *argv[])
{
	...
	printf("PMDK read success, buf_A: %s \n", rootp->buf_A);
    printf("PMDK read success, buf_B: %s \n", rootp->buf_B);
    printf("PMDK read success, buf_C: %s \n", rootp->buf_C);
    printf("PMDK read success, buf_D: %s \n", rootp->buf_D);
    printf("PMDK read success, buf_E: %s \n", rootp->buf_E);
    printf("PMDK read success, buf_F: %s \n", rootp->buf_F);

    // now do some manual consistency checks
    // if see "B" or "C", first write must happen
    if (!strcmp(rootp->buf_B, "B") || !strcmp(rootp->buf_C, "C")) {
        if (!write_1) {
    ...
    if (!strcmp(rootp->buf_D, "D") || !strcmp(rootp->buf_E, "E")) {
        if (!write_2) {
