...
int main(int argc, char *argv[]) {
	...
	string global_log_path = "global_log_file.txt";
	...
	string ops_completed_path = "ops_completed.txt";
	...
	for (int i = 0; i < num_ops; i++) {
		...
		if (!is_fsync && !fsync_hit) {
			...
			if (prob < WRITE_PERCENT) {
				...
				SQUINT_OP_BEGIN(0, i);
				ret = db->Put(write_options, key, value, &is_fsync);
				SQUINT_OP_END(0, i);
				...
				if (fast_delete) {
					if (!keys_inserted.empty()) {
						...
						SQUINT_OP_BEGIN(0, i);
						ret = db->Delete(write_options, del_key, &is_fsync);
						SQUINT_OP_END(0, i);
						assert(ret.ok());
						entry = thread_id + std::to_string(i) + "," + (is_fsync ? "SYNC," : "ASYNC,")
								+ "DELETE," + del_key;
	...
	/* Close the database */
	std::cout << "bug1 workload completed"<< std::endl; 
	global_log.close();
	ops_completed.close();
	delete db;
}