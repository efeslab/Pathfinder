...
int main(int argc, char *argv[]) {
	...
	unordered_map<string, string> expected_state_all_applied; // expected_state all applied
	...
	unordered_map<string, string> all_ops; // map of op_id -> thread_id
	...
	while (std::getline(op_completed, oc_entry)) {
		...
		all_ops.insert({get_entry[1], get_entry[0]});
		if (get_entry[2] == "1") {
	...
	while (std::getline(global_log, gl_entry)) {
		...
		if (get_entry[3] == "PUT") {
			expected_state_all_applied.insert({get_entry[4], get_entry[5]});
		} else if (get_entry[3] == "DELETE") {
			expected_state_all_applied.erase(get_entry[4]);
		}
		...
		if (contains_key(completed_ops, get_entry[1])) {
			...
			if (get_entry[2] == "SYNC") {
				...
				expected_state_last_sync = expected_state_all_applied; 
			}
		}
	}
